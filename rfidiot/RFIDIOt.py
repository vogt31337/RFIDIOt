...
import rfidiot.pynfc as pynfc
import rfidiot.pyandroid as pyandroid
...
try:
    from rfidiot.pynfc import get_version
...
class rfidiot:
    ...
    # local imports
    # from rfidiot.iso3166 import ISO3166CountryCodesAlpha
    ...
    def __init__(self, readernum, reader, port, baud, to, debug, noinit, nfcreader):
        ...
        if not self.NoInit:
            if self.readertype == self.READER_PCSC:
                ...
                if str.find(self.readername, 'OMNIKEY') == 0:
                ...
                try:
                    ...
                    if self.readersubtype == self.READER_ACS and self.pcsc_protocol == smartcard.scard.SCARD_PROTOCOL_T1:
                        ...
                        hresult, hcard, dwActiveProtocol = smartcard.scard.SCardConnect(
                            hcontext, readers[readernum], smartcard.scard.SCARD_SHARE_DIRECT,
                        ...
                        hresult, response = smartcard.scard.SCardControl(self.hcard,
                                                                         IOCTL_SMARTCARD_VENDOR_IFD_EXCHANGE, apdu)
    ...
    def MIFAREkb(self, data):
        ...
        self.MIFAREblock1AC = str((self.MIFAREC1 & 0x02) >> 1) + str((self.MIFAREC2 & 0x02) >> 1) + str(
            (self.MIFAREC3 & 0x02) >> 1)
        self.MIFAREblock2AC = str((self.MIFAREC1 & 0x04) >> 2) + str((self.MIFAREC2 & 0x04) >> 2) + str(
            (self.MIFAREC3 & 0x04) >> 2)
        self.MIFAREblock3AC = str((self.MIFAREC1 & 0x08) >> 3) + str((self.MIFAREC2 & 0x08) >> 3) + str(
            (self.MIFAREC3 & 0x08) >> 3)
